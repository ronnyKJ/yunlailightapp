 ...
 */
	...
	lazy_img : function(){
		...
		lazyNode.each(function(){
			var self = $(this);
			if(self.is('img')){
				self.attr('src','./linxusstatic/loading_large.gif');
			}else{
				// 把原来的图片预先保存下来
				...
				self.css({
					'background-image'	: 'url(./linxusstatic/loading_large.gif)',
 ...
 */
	//插件启动函数
 	plugin : function(){
		...
		// 音符飘逸
		$('#coffee_flow').coffee({
			steams				: ["<img src='./linxusstatic/audio_widget_01@2x.png' />","<img src='/template/19/img/audio_widget_01@2x.png' />"], 
			steamHeight			: 100,
			steamWidth			: 44 
		});
		...
		var node = $('#j-mengban')[0],
			url = './linxusstatic/page_01_bg@2x.jpg',
